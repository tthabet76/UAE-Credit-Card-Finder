...
from update_cards import process_card_data, chromedriver_path, db_file, setup_database
...
cursor = conn.cursor()
cursor.execute("SELECT url, bank_name, card_name FROM card_inventory WHERE is_active = 1 LIMIT 1")
row = cursor.fetchone()
