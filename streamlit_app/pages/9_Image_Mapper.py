...
def load_data():
    ...
    # LEFT JOIN to ensure ALL cards are returned
    query = """
    SELECT 
        d.id, 
        d.bank_name, 
...
def save_uploaded_file(uploaded_file, bank_name, card_name):
    try:
        ...
        sanitized_bank = clean_filename(bank_name)
        sanitized_card = clean_filename(card_name)
        filename = f"{sanitized_bank}-{sanitized_card}.png"
        
        file_path = os.path.join(IMAGE_DIR, filename)
        
        image = Image.open(uploaded_file)
        # Convert CMYK/RGBA if needed
        if image.mode == 'CMYK':
...
if show_unmapped_only:
    # Filter where local_filename is null
...
for index, row in df.iterrows():
    ...
    with st.container():
        ...
        with c2:
            ...
            # FILE UPLOAD
            uploaded_file = st.file_uploader("Upload Image", type=['png', 'jpg', 'jpeg', 'webp'], key=f"up_{card_id}")
            if uploaded_file is not None:
                if st.button("💾 Save Upload", key=f"save_up_{card_id}"):
                    saved_filename = save_uploaded_file(uploaded_file, row['bank_name'], row['card_name'])
                    if saved_filename:
                        save_mapping(card_id, row['bank_name'], row['card_name'], saved_filename, "")
            ...
            if st.button("💾 Save Settings", key=f"save_{card_id}"):
                if new_file == "None":
                    new_file = None
